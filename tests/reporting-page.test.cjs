...
test('PWA application, release metadata and service-worker cache are consistently versioned at 1.0.17', () => {
  const app = loadAppScript();
  ...
  assert.equal(appVersion, '1.0.17');
  assert.equal(versionInfo.version, '1.0.17');
  assert.match(serviceWorker, /spdk-cache-v1\.0\.17/);
});
